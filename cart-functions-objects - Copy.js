...
function addToCart(product, i){
    ...
    var data = []
    data = CART.concat(JSON.parse(localStorage.getItem("cart")));
...
function showCart()
{
    ...
    var elementCounts = cart.reduce((count, item) => (count[item] = count[item] + 1 || 1, count), {});
	...
	for(let i = 0; i < cart.length; i++)   // produce a table row for each item in the cart currently
	{
        prodTotal = 0;

        let prodName = JSON.stringify(cart[i]);
        prodName = prodName.substring(1, prodName.length -1);
        prodName = prodName.replaceAll(String.fromCharCode(92),'');

        cartSlot = JSON.parse(cart[i]);
        //const {...object} = cartSlot

        let discAmt = 0;

        //MAKE-SHIFT INLINE FUNCTION FOR SETTING DISCOUNT SINCE JSON.parse REMOVES INITIAL CLASS FUNCTIONS AND CONSTRUCTORS
        for(let i = 1; i <= elementCounts[prodName]; i++){
            if(i % cartSlot.prodAmtForDiscount == 0){
                discAmt += 5;
            }
        }
        ...
        if(!prodInCart.includes(prodName)){
		    document.write(
                ...
                '</td><td class="numericCol"> <p> '  + elementCounts[prodName] + //PRODUCT QUANTITY
                '</td><td class="numericCol"> <p> '  + cartSlot.productDesc + //PRODUCT DESC
                '</td><td class="numericCol"><p> $'  + cartSlot.price + //PRODUCT PRICE
                '</td><td class="numericCol"><p> $'  + (elementCounts[prodName] * discountProduct(cartSlot.price, discAmt)).toFixed(2) + '<br><br>' + txtDiscApplied + //PRODUCT TOTAL
                '</td></tr>'
            );
	...
	//{       
		...
		document.write(
            ...
            + '</strong></td><td><img src = "purchase.png" class = "smallPic" id = "purchaseBtn"></td></tr></table>'
            );	
